...
const fieldSize = 19; // size of field (don't change)
const cellSize = 30; // size of 1 cell is 30px; (don't change)
...
let line = ""; // empty row
const weights = [
  // array of objects with weights and patterns
  { weight: 10000, pattern: ["xxxxx"] },
  { weight: 5000, pattern: ["0xxxx0"] },
  { weight: 2500, pattern: ["0xxxx", "xxxx0"] },
  ...
  {
    weight: 100,
    pattern: ["0x0xx", "xx0x0", "x0xx0", "0xx0x", "x00xx", "xx00x"],
  },
...
firstMove.fillText(
  moveCount,
  firstMoveCoord * cellSize,
  firstMoveCoord * cellSize + 8
);
isCross = false;
...
for (let i = 0; i < 5; i++) {
  for (let j = 0; j < 5; j++) {
    if (
      firstMoveCoord - 2 + i >= 0 &&
      firstMoveCoord - 2 + i < fieldSize &&
      firstMoveCoord - 2 + j >= 0 &&
      firstMoveCoord - 2 + j < fieldSize
    ) {
      if (
        potentials[firstMoveCoord - 2 + i][firstMoveCoord - 2 + j] != 0 &&
...
function fillPotentials(potentY, potentX) {
  ...
  for (let i = 0; i < 5; i++) {
    for (let j = 0; j < 5; j++) {
      if (
        potentY - 2 + i >= 0 &&
        potentY - 2 + i < fieldSize &&
        potentX - 2 + j >= 0 &&
        potentX - 2 + j < fieldSize
      ) {
        if (
          potentials[potentY - 2 + i][potentX - 2 + j] != 0 &&
...
function checkWin(winY, winX) {
  if (moveCount >= 9) {
    // if amount of moves >= 9 then do the function
    // checking win row
    for (let i = 0; i < 5; i++) {
      ...
      //checking win diagonal top left - bottom right
        ...
        winX + i < fieldSize &&
        winY + i < fieldSize &&
        cells[winY - 4 + i][winX - 4 + i] ==
          cells[winY - 3 + i][winX - 3 + i] &&
        cells[winY - 3 + i][winX - 3 + i] ==
          cells[winY - 2 + i][winX - 2 + i] &&
        cells[winY - 2 + i][winX - 2 + i] ==
          cells[winY - 1 + i][winX - 1 + i] &&
        cells[winY - 1 + i][winX - 1 + i] == cells[winY + i][winX + i]
      ) {
        ctx.beginPath();
        ctx.moveTo((winX - 4 + i) * cellSize, (winY - 4 + i) * cellSize);
        ctx.lineTo(
          (winX + i) * cellSize + cellSize,
          (winY + i) * cellSize + cellSize
        );
        ctx.stroke();
        isFin = true;
      }
...
function compareDefense(direction, weight, cellValue) {
  for (let item of weights) {
    for (let value of item.pattern) {
      ...
      if (direction.search(line) != -1) {
        // compare line with direction string
        if (weight > 1) {
...
function calcAIMove() {
  ...
  if (attackW * 1.1 >= defenseW && isFin == false) {
    // if attack * 1.1 > defense do attack move
